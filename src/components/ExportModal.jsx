...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (bgVideo) {
        ...
        function _vidSetPlaying() {
            _vidAudioUnlocked = true;
            _vidAudioMuted    = false;
            bgVideo.muted     = false;
            _vidUpdateUI();
            _vidRemoveListeners();
        }

        function _vidOnFirstInteraction() {
            if (!_vidAudioUnlocked) {
                bgVideo.muted = false;
                bgVideo.play().then(function() { _vidSetPlaying(); }).catch(function(){});
            }
        }

        function _vidHandleSpeakerTap(e) {
            e.preventDefault();
            e.stopPropagation();
            if (!_vidAudioUnlocked) {
                bgVideo.muted = false;
                bgVideo.play().then(function() { _vidSetPlaying(); }).catch(function(){});
            } else if (_vidAudioMuted) {
                _vidAudioMuted = false;
                bgVideo.muted  = false;
                _vidUpdateUI();
            } else {
        ...
        _vidSpeakerBtn.addEventListener('click',    _vidHandleSpeakerTap);
        _vidSpeakerBtn.addEventListener('touchend', _vidHandleSpeakerTap);

        function _vidRemoveListeners() {
            document.removeEventListener('touchstart',  _vidOnFirstInteraction, true);
            document.removeEventListener('touchend',    _vidOnFirstInteraction, true);
            document.removeEventListener('click',       _vidOnFirstInteraction, true);
            document.removeEventListener('pointerdown', _vidOnFirstInteraction, true);
        ...
        document.addEventListener('touchend',    _vidOnFirstInteraction, { once: true, capture: true });
        document.addEventListener('click',       _vidOnFirstInteraction, { once: true, capture: true });
        document.addEventListener('pointerdown', _vidOnFirstInteraction, { once: true, capture: true });
        document.addEventListener('scroll',      _vidOnFirstInteraction, { once: true, capture: true });
    }
    ` : ''}
